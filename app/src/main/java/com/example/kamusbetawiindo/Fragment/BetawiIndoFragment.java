...
public class BetawiIndoFragment extends Fragment {
    DBHelper dbHelper;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        ...
        btnTranslate.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (pilih_algo == 1){
                    TimingLogger timingLogger = new TimingLogger("TIME", "BOYER MOORE");
                    prosesBoyerMoore();
                    timingLogger.addSplit("BOYER MOORE");
...
//        hasil = getIndonesia(pattern);
    ...
    void prosesView() {
        ...
        if (kamusListTemp.size() > 0) {
            for (int i = 0; i < kamusListTemp.size(); i++) {
                if (pattern.equals(kamusListTemp.get(i).getBetawi())) {
                    etHasil.setText(kamusListTemp.get(i).getIndo().toUpperCase());
                    etHasil.setTextColor(Color.BLUE);
                    matchString = true;
                    Log.d("True", kamusListTemp.get(i).getIndo());
                } else if (pattern.contains(kamusListTemp.get(i).getBetawi())) {
                    Log.d("Mungkin Maksud Anda", kamusListTemp.get(i).getIndo());
                    matchString = false;
                }
            }
            if (matchString == true) {
    ...
    private String getIndonesia(String addText) {
        Kamus kamus = dbHelper.getbetawi(addText);
